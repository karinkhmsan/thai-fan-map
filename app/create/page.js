...
export default function CreatePage() {
  ...
  const handleFiles = (fileList) => {
    const arr = Array.from(fileList).slice(0, 6 - files.length);
      ...
      <div className="card" style={{ padding: 20, display: "grid", gap: 16 }}>
        ...
        <Field label="รูปภาพในงาน / รูปคอสเพลย์ (สูงสุด 6 รูป)">
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            ...
            {images.length < 6 && (
              <div onClick={() => fileRef.current.click()} style={{ width: 84, height: 84, borderRadius: 10, border: "1.5px dashed rgba(255,255,255,0.25)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#8177AE", gap: 4 }}>
                <Plus size={18} /><span style={{ fontSize: 11 }}>เพิ่มรูป</span>
              </div>
            )}
