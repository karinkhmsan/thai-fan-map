...
export default function HomePage() {
  ...
  const eventsByProvince = useMemo(() => {
    ...
    events
      ...
      .forEach((e) => { (map[e.province] = map[e.province] || []).push(e); });
    return map;
  }, [events, catFilter]);
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <button className={`chip ${catFilter === "all" ? "active" : ""}`} onClick={() => setCatFilter("all")}
          style={{ borderColor: catFilter === "all" ? "#7F77DD" : undefined, background: catFilter === "all" ? "#7F77DD22" : undefined, color: catFilter === "all" ? "#7F77DD" : undefined }}>
          ทั้งหมด
        </button>
        {CATEGORIES.map((c) => (
          <button key={c.id} className={`chip ${catFilter === c.id ? "active" : ""}`} onClick={() => setCatFilter(c.id)}
            style={{ borderColor: catFilter === c.id ? c.color : undefined, background: catFilter === c.id ? c.color + "22" : undefined, color: catFilter === c.id ? c.color : undefined }}>
            {c.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ marginTop: 22 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          ...
          {Object.keys(eventsByProvince).length === 0 && !loading && (
